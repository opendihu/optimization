...
# - Linear elasticity
#
# arguments: <scenario_name> <n elements x>
...
if len(sys.argv) > 3:
  ...
  if scenario_name == "compressible_mooney_rivlin":
    material_parameters = [0.5, 0.75, 1.0]      # c1, c2, c
    
  elif scenario_name == "compressible_mooney_rivlin_decoupled":
    material_parameters = [0.5, 0.75, 10.0]      # c1, c2, kappa
    
  elif scenario_name == "incompressible_mooney_rivlin":
    material_parameters = [1.0, 1.0]      # c1, c2
    
  elif scenario_name == "nearly_incompressible_mooney_rivlin":
    material_parameters = [1.0, 1.0, 1e3]      # c1, c2, kappa
...
def handle_result_hyperelasticity(result):
  ...
  if data["timeStepNo"] == 1:
    ...
    with open("result_convergence.csv","a") as f:
      f.write("{},{},{},{}\n".format(scenario_name,nx,strain,stress))
...
def handle_result_febio(result):
  ...
  if data["timeStepNo"] == 1:
    ...
    # field variables: geometry, activation, u, reactionForce, 4:sigma, 5:S, 6:E, 7:J
