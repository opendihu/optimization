...
variables.contraction_dirichlet_bc = {}
variables.prestretch_dirichlet_bc = {}

for j in range(my):
  for i in range(mx):
    variables.prestretch_dirichlet_bc[(mz-1)*mx*my + j*mx + i] = [0.0,0.0,0.0,None,None,None]
    variables.contraction_dirichlet_bc[(mz-1)*mx*my + j*mx + i] = [0.0,0.0,0.0,None,None,None]
    variables.contraction_dirichlet_bc[ j*mx + i] = [0.0,0.0,0.0,None,None,None]
...
variables.ny = ny
variables.nz = nz
variables.prestretch_neumann_bc = [{"element": 0*nx*ny + j*nx + i, "constantVector": variables.bottom_traction, "face": "2-"} for j in range(ny) for i in range(nx)]
...
def callback_function_prestretch(raw_data):
  ...
  if n_ranks==1:
    ...
    f = open("muscle_prestretch_" + "N.csv", "a")
