...
import time
import setup_BayesOpt_biceps_data


"""
This is a file to carry out Bayesian Optimization for a dummy function.
To call this file, run:
>python BayesOpt_biceps_data.py
To customize the optimization process and the target function, change settings in the file "setup_BayesOpt_biceps_data.py".
"""

nu = setup_BayesOpt_biceps_data.nu
matern = setup_BayesOpt_biceps_data.matern
rbf = setup_BayesOpt_biceps_data.rbf

const = setup_BayesOpt_biceps_data.const
zero = setup_BayesOpt_biceps_data.zero

fixed_noise = setup_BayesOpt_biceps_data.fixed_noise
variable_noise = setup_BayesOpt_biceps_data.variable_noise

EI = setup_BayesOpt_biceps_data.EI
PI = setup_BayesOpt_biceps_data.PI
KG = setup_BayesOpt_biceps_data.KG
ES = setup_BayesOpt_biceps_data.ES
ES_low_uncertainty = setup_BayesOpt_biceps_data.ES_low_uncertainty

stopping_y = setup_BayesOpt_biceps_data.stopping_y
improvement_threshold = setup_BayesOpt_biceps_data.improvement_threshold
stopping_xy = setup_BayesOpt_biceps_data.stopping_xy
x_range = setup_BayesOpt_biceps_data.x_range
num_consecutive_trials = setup_BayesOpt_biceps_data.num_consecutive_trials

fixed_Yvar = setup_BayesOpt_biceps_data.fixed_Yvar
lower_bound = setup_BayesOpt_biceps_data.lower_bound
sobol_on = setup_BayesOpt_biceps_data.sobol_on
num_initial_trials = setup_BayesOpt_biceps_data.num_initial_trials
visualize = setup_BayesOpt_biceps_data.visualize
add_points = setup_BayesOpt_biceps_data.add_points
upper_bound = setup_BayesOpt_biceps_data.upper_bound
...
elif ES_low_uncertainty:
    global_individuality_parameter = global_individuality_parameter + "_ES_low_uncertainty"
    title = title + "Entropy Search (modified), "
if stopping_y:
...
for input in initial_x:
    y = torch.tensor([[setup_BayesOpt_biceps_data.target_function(input*(upper_bound-lower_bound)+lower_bound)]], dtype=torch.double)
...
for i in range(num_iterations):
    ...
    if KG:
        ...
        if i%3==0:
            ...
            variance = posterior.variance.squeeze(-1).detach().numpy()
            candidate_index = np.argmax(variance)
            candidate = torch.tensor([[x_query[candidate_index]]])
            print("Used value of maximal variance")
        else:
            bounds=torch.tensor([[0], [1]], dtype=torch.double)
            candidate_set = torch.rand(1000, bounds.size(1))
            candidate_set = bounds[0] + (bounds[1] - bounds[0]) * candidate_set
    ...
    new_y = torch.tensor([[setup_BayesOpt_biceps_data.target_function(candidate[0]*(upper_bound-lower_bound)+lower_bound)]], dtype=torch.double)
...
while continuing == "y":
    ...
    new_y = torch.tensor([[setup_BayesOpt_biceps_data.target_function(candidate[0])]], dtype=torch.double)
    new_yvar = torch.full_like(new_y, fixed_Yvar, dtype=torch.double)
