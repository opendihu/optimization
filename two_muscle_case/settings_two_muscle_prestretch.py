...
for fiber_x in range(fb_x):
    for fiber_y in range(fb_y):
        ...
        meshName = "fiber_1{}".format(fiber_no)
...
for fiber_x in range(fb_x):
    for fiber_y in range(fb_y):
        ...
        y = ny * fiber_y / (fb_y - 1)
        nodePositions = [[x, y, -nz * i / (fb_points - 1)] for i in range(fb_points)]
        meshName = "fiber_2{}".format(fiber_no)
        meshes[meshName] = { # create fiber meshes
            "nElements":            [fb_points - 1],
            "nodePositions":        nodePositions,
            "inputMeshIsGlobal":    True,
...
elasticity_neumann_bc_1 = [{"element": k*nx*ny + j*nx + i, "constantVector": traction_vector_1, "face": "2+"} for j in range(ny) for i in range(nx)]
elasticity_neumann_bc_2 = [{"element": k*nx*ny + j*nx + i, "constantVector": traction_vector_2, "face": "2+"} for j in range(ny) for i in range(nx)]
